    ...
    methods: {
        getError: function(field){
            ...
            if(v && v.$error === true){
                ...
                if(v.minFile === false){
                    if(v.$params && v.$params.minFile && v.$params.minFile.errMsg) {
                        return v.$params.minFile.errMsg
                    }
                }
...
window.customValidators = {
    ...
    minFile: function(min, errMsg) {
        return window.vuelidate.withParams(
            {
                type: 'minFile',
                errMsg: errMsg || 'Select at least ' + min + ' file.'
            },
            function(value){
                return !window.validators.helpers.req(value) || value.length >= min
            }
        )
    },
